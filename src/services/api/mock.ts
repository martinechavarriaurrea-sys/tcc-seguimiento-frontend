...
export const mockDashboard = {
  async getStats(): Promise<DashboardStats> {
    await delay();
    const activas = mockGuias.filter((g) => g.activa);
    return {
      total_activas: activas.length,
      total_guias_activas: activas.length,
      total_entregadas: mockGuias.filter((g) => g.estado_actual === 'entregado').length,
      total_guias_entregadas_hoy: mockGuias.filter((g) => g.estado_actual === 'entregado').length,
      con_novedad: activas.filter((g) => g.estado_actual === 'novedad').length,
      sin_movimiento: activas.filter((g) => g.alertas.some((a) => a.tipo === 'sin_movimiento')).length,
      monitoreadas_hoy: activas.length,
      ultima_ejecucion: '2026-04-28T12:00:00Z',
      proxima_ejecucion: '2026-04-28T16:00:00Z',
      proxima_reporte: '2026-04-28T16:00:00Z',
      guias_activas: activas.map((g) => ({
        numero_guia: g.numero_guia,
        estado_actual: g.estado_actual,
        ultima_actualizacion: g.fecha_ultima_actualizacion,
      })),
      sistema_activo: true,
